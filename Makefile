...
preflight-unit-tests: ## Run the fast unit tests preflight depends on.
	...
	@go test ./internal/controller/ -run 'TestRender_|TestResolveTrainerImage_|TestParseBearerChallenge|TestVerifyAdapterArtifact_|TestParseAgentProposal|TestValidateQLoRAConfig|TestExtractJSONObject|TestRenderAutoresearch' -count=1 >/dev/null \
	  && echo "  OK  render + verify + agent + KB-bootstrap + helper unit tests" \
	  || { echo "  ERR unit tests failed; re-run with: go test ./internal/controller/ -v -run 'TestRender_|TestResolveTrainerImage_|TestParseBearerChallenge|TestVerifyAdapterArtifact_|TestParseAgentProposal|TestValidateQLoRAConfig|TestExtractJSONObject|TestRenderAutoresearch'"; exit 1; }
